...
router.get('/:id', async (req, res) => {
  ...
  router.put('/:id', upload.single('mainPic'), async (req, res) => {
    try {
      ...
      if (req.file) {
        event.mainPic = req.file.buffer; // Store as buffer, not as {data, contentType}
      }
      // Do NOT touch event.mainPic if no file is uploaded
  ...
  // Add this route to your eventRoutes.js file



// Also add a route to delete entire sub-events
router.delete('/:id/subevents/:subEventIndex', async (req, res) => {
  try {
    const event = await Event.findById(req.params.id);
    if (!event) return res.status(404).json({ message: 'Event not found' });
...
// Add this route to update a sub-event by index
router.put('/:id/subevents/:subEventIndex', upload.array('images'), async (req, res) => {
  try {
    const event = await Event.findById(req.params.id);
    if (!event) return res.status(404).json({ message: 'Event not found' });
    ...
    if (req.body.name) event.subEvents[subEventIndex].name = req.body.name;
    if (req.body.description) event.subEvents[subEventIndex].description = req.body.description;
    ...
    if (req.files && req.files.length > 0) {
      ...
      req.files.forEach(file => {
        event.subEvents[subEventIndex].images.push(file.buffer);
      });
    }
